...
function TabDashboard({ orders }) {
  ...
  const topMenus = Object.entries(itemCount).sort((a, b) => b[1] - a[1]).slice(0, 5);
            ...
            {tabs.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-all ${isActive ? "border-orange-500 text-orange-500" : "border-transparent text-gray-500 hover:text-gray-700"}`}>
